...
<%!
	...
	public static final String SUPPLY_ADD_URL = "pages/back/admin/supply/supply_apply_add_pre.action" ;
	public static final String SUPPLY_APPLY_LIST_URL = "pages/back/admin/supply/supply_apply_list.action" ;
	public static final String SUPPLY_LIST_URL = "pages/back/admin/supply/supply_list.action" ;
    ...
    public static final String SCHEDULE_LIST_URL = "pages/back/admin/plan/my_plan_pre.action";
...
<aside class="main-sidebar">
    ...
    <section class="sidebar">
        ...
        <ul class="sidebar-menu">
            ...
            <c:if test="${fn:contains(roleList,'dept') || fn:contains(roleList,'chief')}">
                <li class="treeview ${param.mi==1 ? 'active' : ''}"><a href="${basePath}<%=INDEX_URL%>"> <i
                        class="fa fa-institution"></i> <span>部门管理</span> <i
                        class="fa fa-angle-left pull-right"></i>
                </a>
                    <ul class="treeview-menu">
                        <c:if test="${fn:contains(actionList,'dept:add')}">
                            <li class="${param.msi==11 ? 'active' : ''}">
                                <a href="<%=DEPT_ADD_URL%>"><i class="fa fa-cube"></i>
                                    增加部门</a></li>
                        </c:if>
                        <c:if test="${fn:contains(actionList,'dept:list') || fn:contains(roleList,'chief')}">
                            <li class="${param.msi==12 ? 'active' : ''}">
                                <a href="<%=DEPT_LIST_URL%>"><i class="fa fa-object-group"></i>
                                    部门列表</a></li>
                        </c:if>
                    </ul>
                </li>
            </c:if>
            <c:if test="${fn:contains(roleList,'emp') || fn:contains(roleList,'chief')}">
                <li class="treeview ${param.mi==2 ? 'active' : ''}"><a href="${basePath}<%=INDEX_URL%>"> <i
                        class="fa fa-sitemap"></i> <span>人事管理</span> <i
                        class="fa fa-angle-left pull-right"></i>
                </a>
                    <ul class="treeview-menu">
                        <c:if test="${fn:contains(actionList,'emp:add')}">
                        <li class="${param.msi==21 ? 'active' : ''}"><a href="<%=EMP_ADD_URL%>"><i
                                class="fa fa-user-plus"></i>
                            增加雇员</a></li>
                        </c:if>
                        <c:if test="${fn:contains(actionList,'emp:list') || fn:contains(roleList,'chief')}">
                        <li class="${param.msi==22 ? 'active' : ''}"><a href="<%=EMP_LIST_URL%>"><i
                                class="fa fa-users"></i>
                            雇员列表</a></li>
                        </c:if>
                    </ul>
            ...
            <c:if test="${fn:contains(roleList,'reimbursement') || fn:contains(roleList,'chief')}">
             ...
             <c:if test="${fn:contains(roleList,'resource') || fn:contains(roleList,'chief')}">
                <li class="treeview  ${param.mi==4 ? 'active' : ''}"><a href="${basePath}<%=INDEX_URL%>"> <i
                    ...
                    <ul class="treeview-menu">
                        <c:if test="${fn:contains(actionList,'resource:list')}">
                        <li class="${param.msi==41 ? 'active' : ''}"><a href="<%=RESOURCE_LIST_URL%>"><i
                                class="fa fa-tasks"></i> 资源列表</a></li>
                        </c:if>
                        <c:if test="${fn:contains(actionList,'resource:apply')}">
            ...
            <c:if test="${fn:contains(roleList,'plan') || fn:contains(roleList,'chief')}">
                <li class="treeview  ${param.mi==5 ? 'active' : ''}"><a href="${basePath}<%=INDEX_URL%>"> <i
                        class="fa fa-tachometer"></i>
                    <span>工作调度</span> <i class="fa fa-angle-left pull-right"></i>
                </a>
                    <ul class="treeview-menu">
                        <c:if test="${fn:contains(actionList,'plan:add')}">
                        <c:if test="${lid < 3}">
                        <li class="${param.msi==51 ? 'active' : ''}"><a href="<%=PLAN_ADD_URL%>"><i
                                class="fa fa-calendar"></i> 工作安排</a></li>
                        </c:if>
                        </c:if>
                        <c:if test="${fn:contains(actionList,'plan:list')}">
            ...
            <c:if test="${fn:contains(roleList,'schedule') || fn:contains(roleList,'chief')}">
                <li class="treeview  ${param.mi==6 ? 'active' : ''}"><a href="${basePath}<%=INDEX_URL%>"> <i
                        class="fa fa-odnoklassniki-square"></i>
                    <span>工作计划</span> <i class="fa fa-angle-left pull-right"></i>
                </a>
                    <ul class="treeview-menu">
                        <c:if test="${fn:contains(actionList,'schedule:list')}">
            ...
            <c:if test="${fn:contains(roleList,'finance')}">
                <li class="treeview  ${param.mi==7 ? 'active' : ''}"><a href="${basePath}<%=INDEX_URL%>"> <i
                        class="fa fa-map-o"></i>
                    <span>财务审核</span> <i class="fa fa-angle-left pull-right"></i>
                </a>
                    <ul class="treeview-menu">
                        <c:if test="${fn:contains(actionList,'finance:reimbursement')}">
                        <li class="${param.msi==71 ? 'active' : ''}"><a href="<%=AUDIT_REIMBURSEMENT_LIST_URL%>"><i
                                class="fa fa-puzzle-piece"></i> 报销单审核</a></li>
                        </c:if>
                        <c:if test="${fn:contains(actionList,'finance:supply')}">
            ...
            <c:if test="${fn:contains(roleList,'supply') || fn:contains(roleList,'chief')}">
